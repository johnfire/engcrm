...
export function ReconContactCard({ contact, onCall, onNavigate, onScan }: Props) {
  ...
  return <View style={styles.row}>
    ...
    <View style={styles.metaRow}>
      ...
      {contact.fit_score != null && <Text style={styles.fit}>fit {contact.fit_score}</Text>}
      {!!contact.status && <Text style={styles.status}>{contact.status}</Text>}
    </View>
    <View style={styles.actions}>
      <Button label="Call" onPress={onCall} /><Button label="Navigate" onPress={onNavigate} /><Button label="Scan card" onPress={onScan} />
    </View>
  </View>;
}
