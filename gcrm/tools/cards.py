...
def extract_card_fields(image_bytes: bytes, media_type: str = "image/jpeg") -> dict:
    ...
    try:
        ...
        return {"fields": {"is_card": False, "error": _EXTRACTION_FAILED}, "model": _EXTRACT_MODEL_NAME, "cost_usd": 0.0}
...
def promote_to_person(fields: dict, contact_id: int | None) -> int:
    ...
    `met_at` is not on the card — it's typed on the confirm screen and arrives
